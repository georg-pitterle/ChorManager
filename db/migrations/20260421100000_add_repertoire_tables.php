...
final class AddRepertoireTables extends AbstractMigration
{
    public function up(): void
    {
        ...
        $this->execute(
            'ALTER TABLE songs ADD CONSTRAINT songs_ibfk_1'
        ...
        $this->execute("CREATE TABLE IF NOT EXISTS song_category_assignments (
            ...
            CONSTRAINT song_category_assignments_category_fk FOREIGN KEY (repertoire_category_id)
    ...
    {
        // Die Prüfung muss vor den DROPs stehen: Schlägt sie erst danach an, sind
        // die Repertoire-Tabellen samt Zuordnungen bereits weg, songs.project_id
        // bleibt aber nullable - der Rollback endet auf halbem Weg und lässt sich
        // nicht wiederholen.
        $nullProjectRows = (int) ($this->fetchRow(
            'SELECT COUNT(*) AS count FROM songs WHERE project_id IS NULL'
        )['count'] ?? 0);
