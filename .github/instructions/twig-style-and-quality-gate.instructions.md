...
- Binary operators (`and`, `or`, `not`) must have exactly 1 space on **both** sides. Multi-line boolean expressions are therefore forbidden; instead, extract sub-conditions into intermediate `{% set %}` variables so every operator fits on a single line, e.g.:
  ```twig
