...
class UserController
{
    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        if (!$firstName || !$lastName || !$emailInput || empty($roleIds)) {
            $createService = new ModalFormService('user_create');
            $createService->setError('Bitte fülle alle Pflichtfelder aus (inkl. mind. einer Rolle).', $formData);
            return $response->withHeader('Location', '/users')->withStatus(302);
        }

        // Die Spalte fasst 255 Zeichen; ohne diese Grenze käme ein zu langer Wert
        // als QueryException und damit als 500 zurück statt als Formularhinweis.
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        // Nur der eingereichte Wert wird normalisiert. Die gespeicherte Adresse
        // bleibt unangetastet, wenn das Feld gar nicht bearbeitet werden darf -
        // sonst schriebe ein Speichern, das die Adresse nicht anfasst, sie
        // kleingeschrieben zurück und löste ein `user.email.changed` aus.
        $emailInput = $canEditEmail
            ? trim(InputValidator::asString($data['email'] ?? null))
        ...
        // Die Spalte fasst 255 Zeichen; ohne diese Grenze käme ein zu langer Wert
        // als QueryException und damit als 500 zurück statt als Formularhinweis.
        // Zur Kleinschreibung siehe create() - hier gilt sie nur für den
        // eingereichten Wert, siehe $emailInput oben.
        $email = $canEditEmail ? InputValidator::validateEmail($emailInput) : $emailInput;
        if ($email === null || strlen($email) > 254) {
            $editService = new ModalFormService('user_edit_' . $userId);
