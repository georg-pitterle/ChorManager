...
// Danach als dieses Mitglied anmelden und pruefen, dass es nur das sieht, wozu die Rolle laut ihren
// (aus der DB gelesenen) Rechten berechtigt ist:
//  - serverseitige Durchsetzung: geschuetzte Route -> 403, wenn das Recht fehlt; sonst erreichbar
//  - Navigation: kein Menue-Link in einen Bereich, fuer den das Recht fehlt
...
// DB-Rechte durchsetzt (faengt z. B. ein Recht, das beim Login nicht in die Session geladen wird).
test('Rollen-Autorisierung: jede Rolle sieht nur, wozu sie berechtigt ist', async ({ page, browser }) => {
    ...
    // 2. Tatsaechliche Rechte je Rolle als Quelle der erwarteten Matrix.
    const rolePerms = readRolePermissions();

    // 3. Pro Rolle in frischer Browser-Session anmelden und pruefen.
    for (const member of ROLE_MEMBERS) {
        ...
        // Mitglied (nicht als Admin) anmelden - sonst wuerde /login sofort nach /dashboard leiten.
        await context.clearCookies();
        const memberPage = await context.newPage();
        try {
            await login(memberPage, { email: member.email, password: MEMBER_PASSWORD });
            // Navigation einmal laden (rechtegefiltertes Menue).
            await memberPage.goto('/dashboard');

            for (const route of PROTECTED_ROUTES) {
                const allowed = route.requires.some((flag) => Number(perms[flag]) === 1);

                // (a) Serverseitige Durchsetzung ueber den echten Statuscode (Session-Cookie der
                ...
                if (allowed) {
                    expect(status, `${member.role} sollte ${route.path} erreichen duerfen`).not.toBe(403);
                } else {
                    expect(status, `${member.role} darf ${route.path} NICHT erreichen (erwarte 403)`).toBe(403);
                }

                // (b) Navigation: verbotener Bereich darf nicht verlinkt sein. Da das Menue
                //     serverseitig rechtegefiltert wird, ist bei fehlendem Recht KEIN Link mit
                //     diesem Pfad-Praefix sichtbar. (Erlaubte Bereiche werden nicht auf Praesenz
                //     geprueft, da das Menue u. U. eine andere Route desselben Bereichs verlinkt.)
                if (!allowed) {
