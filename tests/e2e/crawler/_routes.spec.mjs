...
test('Routen-Tabelle loest Gruppen-Praefixe auf', async () => {
    const routes = getRoutes();
    const patterns = routes.map((r) => `${r.method} ${r.pattern}`);

    // $projGroup = $group->group('/projects', ...) in Routes.php definiert
    // ->get('/members', [ProjectController::class, 'listForMembers']) - ohne Praefix-Aufloesung
    // kaeme hier faelschlich "GET /members" heraus (404, da die echte Route
    ...
    // Gruppen-Wurzelseite und ->get('/{id:[0-9]+}', ...) fuer die Detailseite - beide muessen
    // mit dem "/song-library"-Praefix aufgeloest werden.
